...
public:
	...
	{
		return Math.Max(MinimumAge, Game.RNG.Next(PopulationGenerationData[Index].AgeRange.Min, PopulationGenerationData[Index].AgeRange.Max));
	}
	Gender GetGenderForIndex(int Index)
	{
		int randomnum = Game.RNG.Next(0, checked((int)Math.Round(unchecked(PopulationGenerationData[Index].MalePercentage + PopulationGenerationData[Index].FemalePercentage + PopulationGenerationData[Index].FutanariPercentage))));
		Gender GetGenderForIndex;
