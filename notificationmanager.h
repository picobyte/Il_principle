...
public:
	...
	void EnqueueNotification(Person owner, QString text, Color color)
	{
	...
	void EnqueueGlobalNotification(QString key, QString text, Color color)
	{
	...
	void HandleSkillNotification(Person per, QString skill, int change)
	{
		if (per == Game.HeadTeacher)
		{
			if (change > 0)
			{
				EnqueueNotification(per, QString.Format("+{0} {1}", change, skill), Colors.LightGreen);
				return;
			}
			if (change < 0)
			{
				EnqueueNotification(per, QString.Format("{0} {1}", change, skill), Colors.OrangeRed);
			}
		}
	}
	void Dispose()
