...
public:
	...
	{
		Person GetPerson;
		if (!QString.IsNullOrEmpty(PerName) && Game.DictOfPersonNames.ContainsKey(PerName))
	...
	Location GetLocation(QString LocName)
	{
		...
		if (!QString.IsNullOrEmpty(LocName) && Game.TheSchool.DictOfLocation.ContainsKey(LocName))
	...
	{
		Clubs GetClub;
		if (!QString.IsNullOrEmpty(ClubName))
		{
